...
const MuiDashboard = ({ technologies }) => {
  ...
  const categories = useMemo(() => {
    const unique = new Set(technologies.map((t) => t.category));
    return Array.from(unique);
  }, [technologies]);

  return (
    <Box sx={{ flexGrow: 1, borderRadius: 4, overflow: 'hidden', border: (theme) => `1px solid ${theme.palette.divider}` }}>
